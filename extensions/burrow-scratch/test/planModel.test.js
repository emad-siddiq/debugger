...
const cases = {
	...
	// --- the plan -------------------------------------------------------------
	'foundations come first, manifests before lockfiles': () => {
		const plan = buildPlan(project(), { name: 'app', reference: '/ref' });
		assert.strictEqual(plan.stages[0].id, '@foundations');
		assert.deepStrictEqual(plan.stages[0].steps, ['backend/go.mod', 'backend/go.sum', 'web/package.json']);
	},
	'a lockfile is a copy step, never a write step': () => {
		const plan = buildPlan(project(), { name: 'app', reference: '/ref' });
		assert.strictEqual(plan.steps['backend/go.sum'].mode, 'copy');
		assert.strictEqual(plan.steps['backend/go.mod'].mode, 'write');
	},
