...
const cases = {
	...
	'`go mod tidy` is checked by the first package of its module': () => {
		const plan = buildPlan(project(), { name: 'app', reference: '/ref' });
		const tidy = plan.stages.filter((s) => s.checks.some((c) => c.cmd === 'go mod tidy'));
		// Exactly one stage per module, and it is the first Go package in it.
		assert.strictEqual(tidy.length, 1);
