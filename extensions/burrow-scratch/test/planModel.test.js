...
const cases = {
	...
	'a go.mod step is checked on what a go.mod step can deliver': () => {
		const plan = buildPlan(project(), { name: 'app', reference: '/ref' });
		const checks = plan.steps['backend/go.mod'].checks;
		assert.deepStrictEqual(checks.map((c) => c.kind), ['shell', 'exists', 'shell']);
		...
		// `go mod init`'s "already exists".
		assert.strictEqual(checks[0].cmd, '[ -f go.mod ] || go mod init example.com/app');
		assert.strictEqual(checks[0].label, '`go mod init example.com/app` succeeds');
		assert.strictEqual(checks[0].cwd, 'backend');

		// THE POINT OF THE SPLIT. `go mod tidy` used to run here too, gated on a
		// `.go` file beside the go.mod — inputs the plan reaches hundreds of steps
		// later, and in a module whose packages all live in subdirectories, never.
		// So step 1 of the plan could not go green whatever the learner did. No
		// precondition survives on this step: everything it asserts is achievable
		// at the moment it is asked for.
		assert.ok(checks.every((c) => c.needs === undefined));
		assert.ok(!checks.some((c) => (c.cmd ?? '').includes('go mod tidy')));

		// …and the third check is what keeps the split honest: a bare `go mod init`
		// infers a module path from the folder name, which succeeds and is wrong.
		assert.strictEqual(checks[2].label, 'it declares module example.com/app');
		assert.match(checks[2].cmd, /grep -Fqx "module example\.com\/app" go\.mod/);
		assert.strictEqual(checks[2].cwd, 'backend');
		// A silent -q failure is a red verdict with no words. It says what it found.
		assert.match(checks[2].cmd, /echo "go\.mod declares:/);
		// The `go` directive is never asserted: `go mod init` writes the toolchain
		// that ran it, and a reference declaring a newer one is not the learner's
		// to close.
		assert.ok(!checks.some((c) => (c.cmd ?? '').includes('go 1.')));

		// The step still TEACHES the full command — only the verdict moved.
		assert.strictEqual(plan.steps['backend/go.mod'].command, 'go mod init example.com/app && go mod tidy');
	},
	'`go mod tidy` is checked by every package stage of its module': () => {
		const plan = buildPlan(project(), { name: 'app', reference: '/ref' });
		const tidy = plan.stages.filter((s) => s.checks.some((c) => c.cmd === 'go mod tidy'));
		// EVERY Go package stage, because each one brings imports the ones before
		// it did not have. Running it once, early, left merkle's module without
		// chi from `backend/middleware` onwards — 25 stages whose `go build` could
		// not pass and no step in the plan would ever have fixed.
		assert.strictEqual(tidy.length, plan.stages.filter((s) => s.cls === 'go').length);
		assert.strictEqual(tidy[0].id, 'backend/models');
		const check = tidy[0].checks.find((c) => c.cmd === 'go mod tidy');
		assert.strictEqual(check.cwd, 'backend');
		// Still qualified — a package you have not written has nothing to resolve.
		assert.deepStrictEqual(check.needs.dir, 'backend/models');
		assert.deepStrictEqual(check.needs.match, '.go');
		// Before `go build`: tidy writes the requires the build then resolves.
		const labels = tidy[0].checks.map((c) => c.cmd);
		assert.ok(labels.indexOf('go mod tidy') < labels.findIndex((c) => c.startsWith('go build')));
	},
	'a module whose packages are all in subdirectories still gets a tidy check': () => {
		// merkle's `test/` module, in miniature: no .go file beside the go.mod, so
		// anchoring the tidy check on the module ROOT would leave it homeless and
		// rebuild the very bug the split removed, one level up.
		const plan = buildPlan(migrating(), { name: 'app', reference: '/ref' });
		assert.strictEqual(plan.steps['backend/go.mod'].checks.length, 3);
		const tidy = plan.stages.filter((s) => s.checks.some((c) => c.cmd === 'go mod tidy'));
		assert.strictEqual(tidy[0].id, 'backend/internal/migrate');
		// …and never on a stage with no Go in it.
		assert.ok(tidy.every((s) => s.cls === 'go'));
	},
	'a package keeps its testdata, so the stage check has what it reads': () => {
		// Go ignores `testdata/` when building and `go test` reads it at run time —
		// a dependency of the stage's own check that no import graph can see. In
		// the leftovers stage it was planned 1,400 steps after the test that reads
		// it: `read testdata/install_sh.golden.sh: no such file or directory`, on a
		// stage completed exactly as written.
		const plan = buildPlan([...project(),
			file('backend/store/testdata/golden.json', '{"nodes":[]}\n')],
		{ name: 'app', reference: '/ref' });
		assert.strictEqual(plan.steps['backend/store/testdata/golden.json'].stage, 'backend/store');
		const steps = plan.stages.find((s) => s.id === 'backend/store').steps;
		// After the code, never before it: the fixture explains nothing on its own.
		assert.strictEqual(steps[steps.length - 1], 'backend/store/testdata/golden.json');
		assert.ok(steps.indexOf('backend/store/store_test.go') < steps.length - 1);
		// …and no stage of its own is left behind for the directory.
		assert.strictEqual(plan.stages.some((s) => s.id === 'backend/store/testdata'), false);
	},
	...
	'a Go step is checked by parsing that one file, not by building the package': () => {
		...
		// Parsing, and NOT `-l`. Four of merkle's own Go files are not gofmt-clean,
		// so a formatting assertion failed steps written exactly as the reference
		// has them — a check the correct answer cannot pass.
		assert.match(checks[1].cmd, /^gofmt -e "backend\/store\/store\.go" > \/dev\/null$/);
		assert.strictEqual(checks[1].emptyOutput, undefined);
	},
	'a file the reference leaves empty is not asked to be non-empty': () => {
		// merkle carries one: a zero-byte placeholder under `.claude/docs`. The
		// step's correct content IS nothing, so "exists and is not empty" could
		// never go green and the step could never be completed.
		const files = [...project(), { path: 'docs/placeholder.md', text: '', bytes: 0 }];
		const plan = buildPlan(files, { name: 'app', reference: '/ref' });
		const [check] = plan.steps['docs/placeholder.md'].checks;
		assert.strictEqual(check.mayBeEmpty, true);
		assert.strictEqual(check.label, 'the file exists');
		// …and every other file still has to have something in it.
		assert.strictEqual(plan.steps['README.md'].checks[0].mayBeEmpty, undefined);
		assert.strictEqual(plan.steps['README.md'].checks[0].label, 'the file exists and is not empty');
	},
	'the stage that owns a package builds and tests it': () => {
		const plan = buildPlan(project(), { name: 'app', reference: '/ref' });
		const stage = plan.stages.find((s) => s.id === 'backend/store');
		// `-o /dev/null`: `go build` on a main package writes an executable named
		// after it, and `test/cmd/oracle`'s landed on the `test/oracle` DIRECTORY.
		assert.deepStrictEqual(stage.checks.map((c) => c.cmd),
			['go mod tidy', 'go build -o /dev/null ./store', 'go test ./store']);
		assert.strictEqual(stage.checks[0].cwd, 'backend');
	},
	'the module root package builds as .': () => {
		const plan = buildPlan(project(), { name: 'app', reference: '/ref' });
		assert.strictEqual(plan.stages.find((s) => s.id === 'backend').checks[1].cmd, 'go build -o /dev/null .');
	},
	...
	'no generate step can go green on an empty directory': () => {
		// The invariant, which outlived the mechanism. Every one of these commands
		// exits 0 with nothing to work on, so exit 0 cannot be the whole verdict.
		// What guards it differs by step, and MUST: a precondition says "your
		// inputs are not here yet" and is right only when the plan reaches those
		// inputs soon. `npm install` is one step behind its package.json; `go mod
		// tidy` was hundreds of steps behind its Go, which is why go.mod asserts
		// its CONTENT instead — `go mod init` on an empty directory succeeds and
		// infers a module path from the folder name, and that check is what says so.
		const plan = buildPlan([...configs(), ...project()], { name: 'web', reference: '/ref' });
		const lock = plan.steps['web/package-lock.json'].checks.find((c) => c.needs);
		assert.deepStrictEqual([lock.needs.dir, lock.needs.match], ['web', 'package.json']);

		const mod = plan.steps['backend/go.mod'].checks;
		assert.ok(mod.every((c) => !c.needs), 'a go.mod step no longer waits on inputs it comes 593 steps before');
		assert.ok(mod.some((c) => (c.cmd ?? '').includes('grep -Fqx "module example.com/app"')));

		for (const step of Object.values(plan.steps)) {
			if (step.mode === 'generate') {
				const guarded = step.checks.some((c) => c.needs)
					|| step.checks.some((c) => (c.cmd ?? '').startsWith('grep -Fqx'));
				assert.ok(guarded, `${step.id} can pass on an empty directory`);
			}
		}
	},
