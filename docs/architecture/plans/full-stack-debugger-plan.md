...
| **M2** | **DB demoable — pgAdmin integrated.** `burrow.db.openPgAdmin` opens pgAdmin in a webview auto-connected to the single `nodewatch-db` (**the literal "pgAdmin integrated"**). Native `pg` explorer is a deferred follow-up, not on the critical path. | pgAdmin/db | L4 | no |
| **M3** | **Docker activity-bar viewlet.** New `burrow-docker` extension: Containers view shows merkle `infra`/`nodewatch-db` under Compose; start/stop/restart/logs/exec/inspect; Images/Volumes/Networks; daemon-down welcome via context key. | docker | L4 | **tracked†** |
