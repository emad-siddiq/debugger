...
| **M4** | **Frontend intellisense + IntelliJ nav complete. DONE (LSP smoke-verified).** `burrow-core` Cmd+B + `editor.multiCursorModifier:'alt'` (N2). Restored the stripped `typescript-basics` grammar + new `burrow-ts-base` LSP client (bundles `typescript-language-server`+`typescript`, `--stdio` transport) → def/hover/references in `.tsx` (N3); headless smoke on merkle `App.tsx` returned a definition location. Go already worked. | go-to-def | L4 | **tracked** |
| **M5** | **Live Watch DONE; seam → M6.** Fiber inspection now ticks on every React commit (`onCommitFiberRoot` → throttled `inspectUpdate`), FD oracle 0 FAIL. The Request Trace click-through (netlog ⨝ dlv slog on `X-Request-Id`) + editable state (`setHook`) fold into M6, where a live dlv session exists to verify the join. | frontend, backend, three-tier | L4 | no |
| **M6** | **One-click orchestrator DONE (build+unit).** New thin standalone `burrow-fullstack`: status-bar `⚡ Debug Full Stack` fans out db-up-and-wait → `startDebugging(<named go config>)` → FD open live. Ships the vision with no core patch. The consolidated **live Burrow drive** (this + M1–M5, and now M7/M8) runs as ONE final pass after M8. | orchestration | L4 | **tracked** |
