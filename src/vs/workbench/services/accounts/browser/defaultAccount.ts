...
function toDefaultAccountConfig(defaultChatAgent: IDefaultChatAgent | undefined): IDefaultAccountConfig {
	// Burrow ships no default chat agent product configuration: chat is backed by
	// an extension-registered participant. A neutral config keeps every consumer
	// on its existing "no provider available" path — no auth provider, no
	// entitlement or token URLs, so no account polling can ever start.
	if (!defaultChatAgent) {
		return {
			preferredExtensions: [],
