...
// ── prod parity: fonts + design tokens (docs/plans/04 §3) ────────────────────
//
// The harness imports the target's own `src/index.css`, which is *supposed* to
// make isolation look like the app. "Supposed to" is what this checks: read the
// computed body font and every `--*` custom property the app declares, read the
// same two things inside the harness, and report what the harness is missing.
// Catch drift, don't hope — a token that quietly stops reaching isolation makes
// every component in it subtly wrong, and nothing else in the sweep would notice.

/** The font the browser actually resolved, plus the design tokens in scope. */
async function readStyleFacts(browser, url) {
  const page = await browser.newPage({ viewport: { width: 1000, height: 800 } })
  try {
    await page.goto(url, { waitUntil: 'load', timeout: PER_PAGE_MS })
    await page.waitForTimeout(SETTLE_MS)
    return await page.evaluate(() => {
      const body = getComputedStyle(document.body)
      // Custom properties are not enumerable on a computed style, so read the
      // authored declarations out of the stylesheets themselves.
...
/** Compare the app with the harness. Missing tokens and a different font are
 *  drift; EXTRA tokens in isolation are not (the harness adds its own chrome). */
async function checkParity(browser, sampleModule) {
  const app = await readStyleFacts(browser, cfg.targetUrl).catch(() => null)
  const iso = await readStyleFacts(browser, isoUrl(sampleModule)).catch(() => null)
  if (!app || !iso) {
    return { ok: true, skipped: true, note: 'could not read one of the two pages' }
  }
  const missing = app.tokens.filter((t) => !iso.tokens.includes(t))
  const fontOk = app.font === iso.font
  return {
    ok: fontOk && missing.length === 0,
    skipped: false,
    app,
    iso,
    fontOk,
...
const parity = await checkParity(browser, modules[0])
if (parity.skipped) {
  console.log(`  parity: skipped — ${parity.note}`)
} else if (parity.ok) {
  console.log(`  parity: ok — same font (${parity.app.font.split(',')[0]}), ${parity.app.tokens.length} tokens present in isolation`)
} else {
  console.log(`  parity: DRIFT — ${parity.fontOk ? '' : `font app="${parity.app.font}" vs iso="${parity.iso.font}"; `}${parity.missing.length} token(s) missing in isolation`)
}
...
md += `**Gate:** ${nBlank + nThrow === 0 && parity.ok ? '✅ 0 blank, 0 throws, no style drift' : `❌ ${nBlank} blank, ${nThrow} throws${parity.ok ? '' : ', style drift'}`}\n\n`

// Prod parity (§3) — fonts and tokens, app vs isolation.
md += `## Prod parity — fonts + tokens\n\n`
if (parity.skipped) {
  md += `Skipped: ${parity.note}.\n\n`
} else {
  md += `| | App | Isolation |\n|---|---|---|\n`
  md += `| body font | \`${esc(parity.app.font)}\` | \`${esc(parity.iso.font)}\` |\n`
  md += `| body size | ${esc(parity.app.size)} | ${esc(parity.iso.size)} |\n`
  md += `| \`--\` tokens | ${parity.app.tokens.length} | ${parity.iso.tokens.length} |\n\n`
  md += parity.ok
    ? `No drift: isolation resolves the same font and carries every token the app declares.\n\n`
    : `**Drift.** ${parity.fontOk ? '' : `The font differs. `}${parity.missing.length ? `Missing in isolation (${parity.missing.length}): \`${parity.missing.slice(0, 30).join('`, `')}\`${parity.missing.length > 30 ? ' …' : ''}` : ''}\n\n`
}
...
const renderOk = nBlank + nThrow === 0
console.log(`\nVERDICT: ${renderOk && parity.ok ? 'pass (0 blank, 0 throws, no style drift)' : `fail (${nBlank} blank, ${nThrow} throws${parity.ok ? '' : ', style drift'})`}`)
process.exit(renderOk && parity.ok ? 0 : 1)
